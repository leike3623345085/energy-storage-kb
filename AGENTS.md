...
### 关键教训
当收到"执行脚本"类任务时，**必须实际执行**，不能仅解释。

### 判断标准
| 指令类型 | 关键词 | 正确做法 |
|---------|--------|---------|
| 脚本执行 | "执行 python3 xxx.py" | 使用 `exec` 工具运行 |
| 命令运行 | "运行/执行 xxx 命令" | 使用 `exec` 或 `shell` 工具 |
| 信息查询 | "查看/检查 xxx" | 直接回答或读取文件 |
| 分析总结 | "分析/总结 xxx" | 读取数据后分析回复 |

### 自检清单
执行任务前问自己：
1. 用户要我**做**什么？还是**解释**什么？
2. 如果是"做"→ 使用工具执行
3. 如果是"解释"→ 直接回答

### 已记录案例
- **2026-03-06**：日报生成任务失败 → AGENT 只解释未执行 → 已修复任务配置
